...
public class FrameLayoutCompat extends ViewGroup{
    ...
    void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes){
        ...
        final TypedArray a = context.obtainStyledAttributes(
                attrs, R.styleable.CircularFrameLayoutCompat, defStyleAttr, defStyleRes);

        mForegroundGravity = a.getInt(
                R.styleable.CircularFrameLayoutCompat_foregroundGravity, mForegroundGravity);

        final Drawable d = a.getDrawable(R.styleable.CircularFrameLayoutCompat_foreground);
        if (d != null) {
            setForeground(d);
        }

        if (a.getBoolean(R.styleable.CircularFrameLayoutCompat_measureAllChildren, false)) {
            setMeasureAllChildren(true);
        }

        if (a.hasValue(R.styleable.CircularFrameLayoutCompat_foregroundTintMode)) {
            mForegroundTintMode = CircularDrawableHelper.parseTintMode(a.getInt(
                    R.styleable.CircularFrameLayoutCompat_foregroundTintMode, -1), mForegroundTintMode);
            mHasForegroundTintMode = true;
        }

        if (a.hasValue(R.styleable.CircularFrameLayoutCompat_foregroundTint)) {
            mForegroundTintList = a.getColorStateList(R.styleable.CircularFrameLayoutCompat_foregroundTint);
            mHasForegroundTint = true;
        }

        mForegroundInPadding = a.getBoolean(R.styleable.CircularFrameLayoutCompat_foregroundInsidePadding, true);
